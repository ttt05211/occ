...
from causal_se2_occ.training import make_loader_with_state
...
def test_loader_generator_state_resumes_next_epoch_order():
    flat = {"features": torch.arange(12, dtype=torch.float32).view(12, 1)}
    device = torch.device("cpu")
    loader, generator = make_loader_with_state(
        flat, batch_size=4, workers=0, seed=123, shuffle=True, device=device
    )

    # Consume one full epoch and capture the exact sampler-generator state.
    _ = [batch["features"].flatten().tolist() for batch in loader]
    state = generator.get_state()
    uninterrupted_next = [
        batch["features"].flatten().tolist()
        for batch in loader
    ]

    resumed_loader, _ = make_loader_with_state(
        flat,
        batch_size=4,
        workers=0,
        seed=999,
