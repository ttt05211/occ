...
p=argparse.ArgumentParser();p.add_argument('--dataroot',required=True);p.add_argument('--output',required=True);p.add_argument('--max-windows',type=int,default=0);a=p.parse_args();src=NuScenesOcc3D(a.dataroot,split='val');by={float(h):None for h in REPORT_HORIZONS_S};wins=list(src.iter_windows(max_windows=(a.max_windows or None)))
